...
Before Wade makes any decision, it needs the full picture. Context assembly is the process of querying the thread model and producing a **context packet** — a structured summary of everything relevant to the current event.

This is what gets passed to the [Decision Layer](04-decision-layer.md). The quality of decisions Wade makes is directly proportional to the quality of context assembly.
...
```
{
  ...
  wade_notes: [
    // context notes Wade has written to itself on this thread
  ],
  current_event: {
...
- **Authored by Wade**: always include Wade's prior actions and notes on this thread
...
Assembly is the main cost center in Wade's processing loop. The query over the thread model should be optimized. Summaries of older history can be cached and updated incrementally rather than rebuilt from scratch each time.
...
**Stale context.** If external state has changed (a PR was merged, a Slack message was sent) and the event stream hasn't caught up, Wade may act on outdated information. Assembly should note the freshness of each piece of context.
